 ...
 * @param {object} [options.manifest] 覆盖写入代码仓的 `xiaomai.content.json`
 */
function createFixture({ contentRepoGit = true, manifest } = {}) {
	const code = mkdtempSync(join(tmpdir(), "xiaomai-export-code-"));
	const content = mkdtempSync(join(tmpdir(), "xiaomai-export-content-"));
...
describe("diffConfig：deepMerge 的精确逆运算", () => {
	...
	it("标量差异整体写出", () => {
		assert.equal(diffConfig("XiaoMai", "我的博客"), "我的博客");
...
describe("content:export 安全闸门", () => {
	it("local 模式拒绝执行，并说明原因", () => {
		const root = mkdtempSync(join(tmpdir(), "xiaomai-export-local-"));
		fixtures.push(root);
		initRepo(root);
	...
	it("导出目标不存在时拒绝：export 不负责创建内容仓", () => {
		...
		const output = expectFailure(() =>
			exportRun(code, [
				"--yes",
				"--out",
				join(tmpdir(), "xiaomai-does-not-exist"),
			]),
		);
	...
	it("content.lock.json 指向别处时告警，提示可能指错仓库", () => {
		const { code } = createFixture();
		const other = mkdtempSync(join(tmpdir(), "xiaomai-export-other-"));
		fixtures.push(other);
...
`,
	...
	it("只增改不删键：已等于默认值的键被保留，--prune-config 才清理", () => {
		...
		assert.match(read(content, "config/site.yaml"), /title: XiaoMai/);
	...
	music: { token: "ghp_0123456789abcdefghijklmnopqrstuvwxyz" },
...
`,
		...
		assert.match(output, /music\.yaml's token/);
		...
		// 告警不阻断：文件照写。
		assert.equal(existsSync(join(content, "config/music.yaml")), true);
	});
...
`,
	...
	it("绝不改动代码仓的 .gitignore、git 索引与 xiaomai.content.json", () => {
		const { code } = createFixture();
		write(code, "src/content/posts/new.md", "# New\n");
		const gitignore = read(code, ".gitignore");
		const manifest = read(code, "xiaomai.content.json");
		const index = git(code, ["diff", "--cached", "--name-only"]);

		exportRun(code, ["--yes"]);

		assert.equal(read(code, ".gitignore"), gitignore);
		assert.equal(read(code, "xiaomai.content.json"), manifest);
