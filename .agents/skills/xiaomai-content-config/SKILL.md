---
name: xiaomai-content-config
description: "配置 XiaoMai 内容分离与外部内容仓库。当搭建或变更 CONTENT_DIR、CONTENT_REPO_URL、CONTENT_REPO_REF、xiaomai.content.json、内容仓库挂载、config/*.yaml 覆盖层、来源优先级或内容仓库配置校验时使用。配置就绪后的日常 sync、export、clean、status 与部署操作请交由 xiaomai-content-workflow。"

---

# XiaoMai 内容分离配置

本技能负责把代码仓和内容仓正确连接，并让内容仓的 YAML 覆盖可验证、可升级。它不承担日常同步、导出、清理或部署执行；这些操作交给 `xiaomai-content-workflow`。
...
6. `xiaomai.content.example.json`：创建或审阅内容源清单时读取。

## 配置顺序

1. 识别目标模式：没有内容源时保留 `local`；需要将文章、数据和用户配置脱离代码仓时使用 `external`。
2. 为外部内容源选择一种入口：本地目录使用 `CONTENT_DIR`；远端仓库使用 `CONTENT_REPO_URL` 与可选的 `CONTENT_REPO_REF`；需要提交到仓库的可复现配置时使用根目录 `xiaomai.content.json`。以权威文档规定的优先级为准，不同时维护互相矛盾的来源。
...
仅在来源、映射和 YAML 均通过验证后，再交给 `xiaomai-content-workflow` 执行 `content:sync`、`content:watch`、`content:export`、`content:clean` 或双仓部署。涉及主题源码或配置领域扩展时，同时遵守 `xiaomai-config` 与 `docs/packaging-contract.md` 的双模式约束。
