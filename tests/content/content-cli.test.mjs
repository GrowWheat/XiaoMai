...
function runCli(args = [], { cwd } = {}) {
	const result = spawnSync(process.execPath, [CLI_SCRIPT, ...args], {
		cwd,
		encoding: "utf8",
		env: { ...process.env, XIAOMAI_CONTENT_SYNC: "0" },
	});
	return {
...
describe("content CLI 总入口与帮助指令", () => {
	it("无参数、--help、-h 或 help 时打印全景帮助文档", () => {
		for (const flag of [[], ["--help"], ["-h"], ["help"]]) {
			...
			assert.match(res.stdout, /XiaoMai Content Separation CLI/i);
	...
	it("子命令分发：status 诊断输出", () => {
		const root = mkdtempSync(join(tmpdir(), "xiaomai-content-cli-"));
		try {
			...
			assert.equal(res.status, 0);
			assert.match(res.stdout, /XiaoMai Content Separation Status & Connectivity Report/);
