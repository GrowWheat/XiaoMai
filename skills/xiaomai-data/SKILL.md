---
name: xiaomai-data
description: "管理 XiaoMai 数据驱动页面与相册——新增或编辑相册目录、info.json 元数据、本地或外部照片,以及 friends、compass、anime、projects、skills、devices、timeline 与 music 数据。当变更 src/content/ 之外供给这些页面的内容时使用。"
---
...
- 文章和动态写入 `src/content/`，使用 `xiaomai-writing`。
- 页面行为、开关、排序和凭据写入 `src/config/`，使用 `xiaomai-config`。
...
3. 新增条目后运行 `npx.cmd astro check`、`pnpm.cmd check:manifest`，并执行对应页面测试，例如 `tests/site/friends.spec.ts`、`tests/site/compass.spec.ts`；其余数据页测试遵循 `tests/site/<domain>.spec.ts` 命名（若该文件存在）。
...
运行 `npx.cmd xiaomais init` 后，用户数据生成在 `xiaomais/config/data/` 和 `xiaomais/content/`，相册及其他静态媒体仍位于用户项目的 `public/`。不要编辑 `node_modules/xiaomais` 中的包文件；构建期消费数据路径时遵循 `docs/npm-package-mode.md` 和 `docs/packaging-contract.md`。
