...
test.describe("文章加密安全与防泄密", () => {
	test("静态 HTML 源码不包含任何未授权正文内容", async ({ page }) => {
		const response = await page.goto("/posts/加密示例/", {
		...
		expect(html).toContain("post:加密示例");
	});
...
test.describe("文章加密交互与解密流", () => {
	test("提供完整的密码验证、解密渲染、TOC样式与会话持久化", async ({
		...
		await page.goto("/posts/加密示例/", { waitUntil: "networkidle" });
		...
		// 正确密码提交
		await input.fill("xiaomai-secret");
		await submit.click();
		...
		const sessionRaw = await page.evaluate(() =>
			sessionStorage.getItem("xiaomai:protected:v1:post%3A加密示例"),
		);
		...
		await page.click('.m3-blog-archive a[href*="加密示例"]');
		await expect(page).toHaveURL(/.*\/posts\/加密示例\/?/);
	...
	test("移动端解密后悬浮 TOC 与侧栏同步并支持锚点跳转", async ({ page }) => {
		...
		// 解锁文章
		await page.locator('input[name="password"]').fill("xiaomai-secret");
