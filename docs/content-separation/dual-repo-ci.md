...
3. 打开该文件，将 `CONTENT_REPOSITORY: OWNER/xiaomai-content` 替换为你自己的内容仓库；
...
- **精准路径过滤**：仅当推送改动涉及文章、数据、图片或配置文件时才触发跨仓构建，修改说明文档或draft不会浪费 Actions 额度；
...
pnpm content:eject --yes    # 实际执行（默认导出到 ../xiaomai-content）
...
4. **生成本地内容源指向**：在代码仓生成 `xiaomai.content.json`，将内容源自动指向导出的外部目录。
...
git remote add origin https://github.com/OWNER/xiaomai-content.git
git push -u origin main
```

### 第三步：配置自动化工作流与密钥

1. **启用代码仓部署流水线**：在代码仓将 `.github/workflows/deploy.yml.example` 复制或重命名为 `deploy.yml`（去掉 `.example` 后缀），并将里面的 `CONTENT_REPOSITORY` 修改为你的内容仓库名（如 `OWNER/xiaomai-content`）；
