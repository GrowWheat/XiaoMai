 ...
 * So in plugin mode we emit subsets into `<projectRoot>/.xiaomais/fonts/`
...
export function fontCacheDir(paths: ResolvedXiaoMaisPaths): string {
...
export async function runFontSubsetting(
	...
	for (const variant of localVariants) {
		...
		if (!source) {
			throw new Error(
				`[xiaomais] Source font file not found: ${variant.file}. ` +
			...
			declarations.push({
				...
				options: {
					variants: localVariants.map((variant) => {
						...
						if (!src) {
							throw new Error(`[xiaomais] Missing font file for "${file}".`);
						}
						return {
							src: [src],
