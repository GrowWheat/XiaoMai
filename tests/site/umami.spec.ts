...
test.describe("Umami analytics", () => {
	...
	test("disabled mode has no analytics DOM, runtime, or requests", async ({
		...
		await expect(page.locator("[data-xiaomai-umami]")).toHaveCount(0);
		await expect(page.locator("style[data-xiaomai-umami-runtime]")).toHaveCount(
			0,
		);
		await expect(
		...
		test(`enabled ${viewport.name} UI is SSR-visible and stable while values load`, async ({
			...
			await page.addInitScript(() => {
				...
				const target = window as Window & {
					__xiaomaiCaptureLayoutShift?: boolean;
					__xiaomaiUmamiLayoutShift?: number;
				};
				target.__xiaomaiUmamiLayoutShift = 0;
				new PerformanceObserver((list) => {
					if (!target.__xiaomaiCaptureLayoutShift) return;
					for (const entry of list.getEntries()) {
						...
						if (!shift.hadRecentInput) {
							target.__xiaomaiUmamiLayoutShift =
								(target.__xiaomaiUmamiLayoutShift ?? 0) + shift.value;
						}
					}
			...
			const stats = page.locator(
				`[data-xiaomai-umami][data-umami-path="${POST_PATH}"]`,
			);
			await expect(stats).toBeVisible();
			await expect(stats.locator("[data-xiaomai-umami-pageviews]")).toHaveText(
				"--",
			);
			await expect(stats.locator("[data-xiaomai-umami-visits]")).toHaveText(
				"--",
			);
			...
			await page.evaluate(() => {
				...
				).__xiaomaiCaptureLayoutShift = true;
			});
			releaseStats();
			await expect(stats).toHaveAttribute("data-umami-loaded", "true");
			await expect(
				stats.locator("[data-xiaomai-umami-display]"),
			).toHaveAttribute("aria-busy", "false");
			await expect(
				stats.locator("[data-xiaomai-umami-pageviews]"),
			).toHaveAttribute("title", "98765");
			await expect(
				stats.locator("[data-xiaomai-umami-visits]"),
			...
			expect(
				await page.evaluate(
					() =>
						(window as Window & { __xiaomaiUmamiLayoutShift?: number })
							.__xiaomaiUmamiLayoutShift ?? 0,
				),
			).toBe(0);
		});
	}
	...
	test("profile metrics stay on one line", async ({ page }) => {
		...
		const display = page.locator(
			".xiaomai-umami-stats--profile [data-xiaomai-umami-display]",
		);
	...
	test("enabled runtime loads article stats after Swup navigation", async ({
		...
		await expect(
			articleStats.locator("[data-xiaomai-umami-pageviews]"),
		).toHaveAttribute("title", "98765");
	});
});
