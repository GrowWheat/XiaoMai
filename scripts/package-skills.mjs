...
function pluginManifest() {
	const authorName =
		typeof packageJson.author === "string"
			? packageJson.author
			: (packageJson.author?.name ?? "XiaoMai contributors");
	return {
		name: pluginName,
		version: pluginVersion,
		description:
			"XiaoMai theme skills for AI coding assistants and content workflows.",
		author: { name: authorName },
		repository: packageJson.repository?.url,
		license: packageJson.license,
		keywords: ["xiaomai", "astro", "svelte", "blog-theme"],
		skills: "./skills/",
		interface: {
			displayName: "XiaoMai AI Skills",
			shortDescription: "Skills for developing and using the XiaoMai theme.",
			longDescription:
				"A single installable bundle containing XiaoMai developer and user-facing skills.",
			developerName: "XiaoMai contributors",
			category: "Productivity",
			capabilities: ["Write", "Code"],
			defaultPrompt: ["Help me work with the XiaoMai theme."],
		},
	};
}
...
function preparePlugin(destination, skills, force = false) {
	...
	if (existsSync(resolvedDestination)) {
		const marker = join(resolvedDestination, ".xiaomai-skills-package");
	...
	writeFileSync(
		join(resolvedDestination, ".xiaomai-skills-package"),
