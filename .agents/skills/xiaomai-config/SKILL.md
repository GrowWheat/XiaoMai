---
name: xiaomai-config
description: "配置 XiaoMai 博客站点——站点身份、主题色、导航、侧栏、页面、评论、Umami 统计、音乐、番剧来源、字体、llms.txt、npm 包初始化以及构建/部署。当启用或调优功能、变更站点行为、准备部署或配置消费 xiaomais 的项目时使用。"
...
| 页面/数据页配置                                              | 技能/项目/时间线/设备等页开关与分类由对应 `src/config/*Config.ts` 与内容仓 `data/*.ts` 共同决定；友链、罗盘、番剧和音乐数据也在内容仓 data 目录维护 |
...
修改该值前，检查已有文章的精确时间在新时区下是否仍落在其 `published`（以及可选的 `updated`）日期；否则构建会报错。仅有 `published: YYYY-MM-DD` 的既有内容不受影响。文章字段的写法和同日排序规则见 `xiaomai-writing`。
...
5. **开发服务器**：使用 `xiaomais()` 的项目修改 Umami 配置后要重启 Astro dev server。集成会在启动时读取配置，已经运行的进程不会因热更新重新创建集成。
...
运行 `npx.cmd xiaomais init` 后，用户配置位于 `xiaomais/config/`，内容位于 `xiaomais/content/`。不要编辑 `node_modules/xiaomais/src/`；包模式的路径和覆盖规则以 `docs/npm-package-mode.md`、`docs/packaging-contract.md` 为准。修改主题默认值时遵循 `rules/project-rules.md` 第 12 节，并同时验证源码模式和包模式。
