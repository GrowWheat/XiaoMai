...
const CLEAN_EXCLUDES = Object.freeze([	"/public/assets/moments/thumbnails/**",
	"/src/assets/fonts/.subset/**",	"**/.gitkeep",
...
function cacheTargets() {
	return [
		...
		{ path: "node_modules/.cache/xiaomai", label: "Config validation cache and digest" },
	];
}
 ...
 *
 * 若清单把某个挂载点指向 `src` 或仓库根，`-x` 会把开发者尚未提交的源码draft、
...
if (resolved.mode === "external") {
	...
	warn(
		`Content source is still active (source: ${origin}). Next 'pnpm dev' or 'pnpm build' will immediately re-materialize content. ` +
			"To stay in local mode permanently, remove this source or set XIAOMAI_CONTENT_SYNC=0.",
	);
}
