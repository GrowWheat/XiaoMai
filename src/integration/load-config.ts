...
import type { ResolvedXiaoMaisPaths } from "./types.ts";
import {
 ...
 * `<projectRoot>/.xiaomais/loaded/`, keyed by a hash of its own contents so
...
function outputDir(paths: ResolvedXiaoMaisPaths): string {
...
export async function loadModuleFile(
	...
	const result = await build({
		...
		banner: {
			js:
				"import { createRequire as __xiaomaisCreateRequire } from 'node:module';\n" +
				"const require = __xiaomaisCreateRequire(import.meta.url);",
		},
...
export async function loadConfigModule(
	...
	if (!entry) {
		throw new Error(
			`[xiaomais] Could not find config module "${name}".\n` +
				`  Looked in: ${paths.configDir}\n` +
				`         and: ${join(paths.packageSrc, "config")}\n` +
				"  Run `npx xiaomais init` to scaffold the default configuration.",
		);
	}
...
export async function loadPackageModule(
	...
	if (!entry) {
		throw new Error(`[xiaomais] Package module not found: src/${relativePath}`);
	}
...
export async function loadConfigValue<T>(
	...
	if (!(exportName in module)) {
		throw new Error(
			`[xiaomais] Config module "${moduleName}" does not export "${exportName}".`,
		);
	}
