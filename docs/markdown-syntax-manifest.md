...
`src/plugins/markdown/manifest.json` 是 XiaoMai 已支持的作者级 Markdown 扩展语法的机器可读单一索引。本指南解释清单边界和维护流程；具体 AST、CSS、缓存与测试规则仍以 `docs/markdown-extensions.md` 为准。
...
`source` 只描述实现来源：`xiaomai` 表示项目自研，`integration` 表示由已接入的 Markdown/渲染生态能力提供。它不改变兼容级别。
