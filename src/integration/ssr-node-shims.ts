 ...
 */
export function xiaomaisSsrNodeShims(): Plugin {
	let isSsr = false;

	return {
		name: "xiaomais:ssr-node-shims",
		apply: "build",
		enforce: "post",
		/**
		...
		renderChunk(code) {
			...
			if (needsFilename || needsDirname) {
				lines.push(
					'import { fileURLToPath as __xiaomaisFileURLToPath } from "node:url";',
				);
			}
			if (needsDirname) {
				lines.push('import { dirname as __xiaomaisDirname } from "node:path";');
			}
			if (needsRequire) {
				lines.push(
			...
			if (needsFilename) {
				lines.push(
					"const __filename = __xiaomaisFileURLToPath(import.meta.url);",
				);
			}
			if (needsDirname) {
				lines.push(
					`const __dirname = __xiaomaisDirname(${filenameDeclared ? "__filename" : needsFilename ? "__filename" : "__xiaomaisFileURLToPath(import.meta.url)"});`,
				);
			}
			if (needsRequire) {
				lines.push("const require = __xiaomaisCreateRequire(import.meta.url);");
			}
