---
name: xiaomai-component-dev
description: "在 XiaoMai 主题中创建或修改 UI 组件——原子设计分层、M3E 令牌、组件 API、图标、动效与无障碍。当需要在 src/components/ 下新增、修改、移动或重构任何内容时使用。"
---
...
6. **文案不硬编码**:用户可见文案走 `src/i18n/i18nKey.ts` + `i18n()`(见 `xiaomai-i18n` 技能)。
...
组件和布局修改必须同时适用于仓库 checkout 与 `node_modules` 中的 `xiaomais`。遵循 `src/integration/overlay.ts` 的覆盖边界，保持包模式 alias 同步，并避免用 `process.cwd()` 读取主题自有文件。双模式检查清单见 `docs/packaging-contract.md` 和 `rules/project-rules.md` 第 12 节。
