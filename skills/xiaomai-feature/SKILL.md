---
name: xiaomai-feature
description: "在零额外负担原则下为 XiaoMai 主题新增可选功能、第三方集成或新配置域——配置/数据分离、按需加载与远程数据契约。当新增评论、统计、widget、远程数据源或新的可开关能力时使用。"
---

# XiaoMai 可选功能与第三方集成

XiaoMai 的可选能力(评论、统计、音乐、追番、页脚注入等)遵循**零额外负担**(Zero Cost when Disabled)原则,原则定义在 `rules/project-rules.md` §7,落地方法以 `docs/on-demand-loading.md` 为准(参考实现:评论系统)。
...
4. **配置与数据分层**:`src/config/*Config.ts` 管行为(开关/分类/排序/凭据),`src/data/*.ts` 管内容实体,两者在构建期协同把行为规则应用到数据;新配置项的类型进 `src/types/<domain>Config.ts`,字段注释就是文档,barrel 注册见 `src/config/README.md`。
