...
function createFixture() {
	const base = mkdtempSync(join(tmpdir(), "xiaomai-content-"));
...
describe("content sync", () => {
	...
	it("XIAOMAI_CONTENT_SYNC=0 可强制回到 local 模式", () => {
		const fixture = createFixture();
		try {
			const { stdout } = runSync(fixture, {
				env: { XIAOMAI_CONTENT_SYNC: "0" },
			});
			assert.equal(stdout, "");
			assert.ok(exists(fixture, "src/content/posts/legacy-demo.md"));
		} finally {
	...
	it("构建期生成物不因共享顶层段而被误删", () => {
		...
		try {
			runSync(fixture);
			// public/assets 顶层段被内容仓拥有，但缩略图是生成物。
			assert.ok(
