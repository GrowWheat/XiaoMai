...
> 本文介绍 XiaoMai 博客主题的「代码仓」与「内容仓」分离解耦架构、目录映射规范、同步机制与日常开发指南。  
...
2. **私密性难以保障**：如果你想把博客代码开源，就不得不将自己的私人日记、draft、好友联系方式一同公开；如果设为私有，又无法享受开源社区带来的便利。

**XiaoMai 的内容分离架构**将博客拆分为两个独立的仓库：
- **代码仓库（公开/Fork）**：存放 XiaoMai 主题源码、组件库与构建引擎，随时可以一键同步拉取上游主题的最新更新；
...
| `external`（双仓解耦模式） | 配置了本地目录（`CONTENT_DIR`）或远端 Git 内容仓（`CONTENT_REPO_URL` / `xiaomai.content.json`） | 真正的“文章只在内容仓写”。在本地预览或线上构建前，系统会自动将外部内容仓的数据安全同步至代码仓标准路径 |
...
```text
xiaomai-content/
├── xiaomai.content.json        # 可选：内容仓自定义元数据与挂载配置
...
如果你在代码仓的某个挂载目录中放置了属于代码仓自己的特殊文件，可以在 `xiaomai.content.json` 中配置 `keep` 白名单，同步时将跳过对这些文件的处理：
...
### 1. 清单文件 (`xiaomai.content.json`)

位于代码仓根目录的配置文件（配置项参考 `xiaomai.content.example.json`）：
...
系统会自动读取代码仓根目录下的 `.env` 与 `.env.local` 文件，配置优先级为：`进程环境变量` > `.env.local` > `.env` > `xiaomai.content.json`。
...
| `XIAOMAI_CONTENT_SYNC` | 设为 `0` 或 `false` 时可强制关闭同步，临时回到单仓模式 |
...
1. **配置本地路径**：在代码仓根目录新建 `.env` 文件，指定你的外部内容仓路径：
   ```bash
   # .env
   CONTENT_DIR="G:/Code/Blog/xiaomai-content"
   ```
