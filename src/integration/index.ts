...
import { xiaomaisFallbackResolver } from "./fallback-resolver.ts";
...
import { xiaomaisOverlay } from "./overlay.ts";
...
import { xiaomaisSsrNodeShims } from "./ssr-node-shims.ts";
import type { ResolvedXiaoMaisPaths, XiaoMaisOptions } from "./types.ts";
...
// it from the dedicated `xiaomais/collections` entry point instead.

const MUSIC_VIRTUAL_ID = "virtual:xiaomai-music-sidebar";
 ...
 */
function createAliases(paths: ResolvedXiaoMaisPaths) {
	...
	return [
		{
			find: "@xiaomai/iconify-offline",
			replacement: join(iconifyDist, "OfflineIcon.svelte"),
		},
		{
			find: "@xiaomai/iconify-offline-functions",
...
function createMusicSidebarPlugin(
	...
	return {
		name: "xiaomais:optional-music-sidebar",
...
/**
 * The XiaoMai theme, packaged as an Astro integration.
 *
 * ```js
 * // astro.config.mjs
 ...
 *   integrations: [xiaomais()],
 * });
 * ```
 */
export function xiaomais(options: XiaoMaisOptions = {}): AstroIntegration {
	let paths: ResolvedXiaoMaisPaths;
				...
				if (paths.isPluginMode && !existsSync(paths.configDir)) {
					logger.warn(
						`No configuration found at ${paths.configDir}. ` +
							"Run `npx xiaomais init` to scaffold it.",
					);
				}
				...
				updateConfig({
					...
					vite: {
						...
						plugins: [
							...
							xiaomaisFallbackResolver(paths),
							xiaomaisSsrNodeShims(),
 ...
 * a fresh project only needs `integrations: [xiaomais()]`.
 */
