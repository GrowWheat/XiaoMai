 ...
 * XiaoMai CLI.
 *
 *   npx xiaomais init            scaffold config, content and static assets
 *   npx xiaomais init --update   restore missing files on an existing project
 *   npx xiaomais init --force    replace template files after backing them up
 *   npx xiaomais info            detailed status and drift report
 *
...
async function readPackageName() {
	try {
		...
		return JSON.parse(raw).name ?? "xiaomais";
	} catch {
		return "xiaomais";
	}
}
...
async function printCliNotice() {
	...
	console.log(`
${colours.bold}XiaoMai${colours.reset} · package manager
${colours.dim}  xiaomais uses ${pmPin} — the version shipped with this release.
  \`init\` sets up pnpm projects only; if npm or yarn is your jam, please migrate manually. Your lockfile, your adventure.
  If you like the project, give https://github.com/yCENzh/xiaomais a star. It helps keep the maintainer's terminal pleasantly quiet.${colours.reset}
`);
}
 ...
 *
 * `copyEntry` is all-or-nothing, which is right for `xiaomais/` but wrong for
 ...
 * `.xiaomais-backup/` first, so even an explicit refresh never destroys the
 * previous copy.
 */
...
async function ensurePackageJson(packageName) {
	...
	if (!existsSync(pkgPath)) {
		...
		const pkg = {
			name: basename(CWD) || "xiaomai-site",
			version: "0.0.1",
			private: true,
			type: "module",
 ...
 * user already ran `pnpm add xiaomais` beforehand.
 */
 ...
 * `@/types/config` resolves while editing files under `xiaomais/config/`.
 */
...
const BACKUP_DIR = ".xiaomais-backup";
...
async function ensureAstroConfig(packageName, { force }) {
	...
	for (const name of present) {
		...
		const wired = current.includes(packageName) || current.includes("xiaomais");
...
async function checkAndUpdate(packageName, { apply }) {
	...
	if (clean) {
		...
		if (!apply) {
			console.log(`\n${colours.dim}Nothing was changed. Run \`npx xiaomais init --update\` to restore the missing files and refresh the scaffold.${colours.reset}\n`);
			return;
		}
...
async function init(args) {
	...
	console.log(`\n${colours.bold}XiaoMai${colours.reset} · initialising project`);
	...
	// config they want from `.xiaomais-backup/`.
	if (force) {
...
async function info() {
	...
	console.log(`\n${colours.bold}XiaoMai project info${colours.reset}`);
	section("Package");
	...
	if (!initialized) {
		console.log(`\n${colours.dim}Run \`npx xiaomais init\` to scaffold this project.${colours.reset}\n`);
		return;
	}
	...
	if (total === 0) {
		...
		console.log(`    ${colours.dim}Run \`npx xiaomais init --update\` to restore safe missing files.${colours.reset}`);
	}
	console.log();
}
...
function help() {
	console.log(`
${colours.bold}XiaoMai CLI${colours.reset}
