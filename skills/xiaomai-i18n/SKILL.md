---
name: xiaomai-i18n
description: "为 XiaoMai 主题新增或变更面向用户的 UI 文案——I18nKey 枚举、十个 locale 模块、占位符一致性与译文查找。当引入、重命名或编辑界面字符串,或新增支持语言时使用。"
---

# XiaoMai 国际化(i18n)

XiaoMai 禁止硬编码用户可见组件文案:所有 UI 字符串走 `src/i18n/i18nKey.ts` 的 `I18nKey` 枚举注册,由消费方用 `i18n()` 按 `siteConfig` 的语言渲染。
