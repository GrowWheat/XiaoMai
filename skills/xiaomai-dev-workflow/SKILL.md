---
name: xiaomai-dev-workflow
description: "XiaoMai Astro 博客主题的日常开发工作流——环境搭建、验证门禁、缓存清理、提交约定、npm 包模式冒烟测试与发布校验。当构建、测试、校验、格式化、提交或准备 XiaoMai 包发布时使用。"
---
...
XiaoMai 是 Astro 7 + Svelte 5 + Tailwind 4 + Stylus + pnpm 的 M3E 博客主题,开发环境为 Windows。本技能覆盖日常开发的环境、验证与提交流程;架构与组件规范见 `xiaomai-component-dev` 等专项技能。
...
- 视觉回归快照仅存本地(已 gitignore);确认每处差异都是有意为之才更新,不吸收无关的页高/环境漂移。
...
XiaoMai 同时支持源码 checkout 和已发布的 `xiaomais` npm 包。修改主题源码前先阅读 `docs/npm-package-mode.md`、`docs/packaging-contract.md`，并遵循 `rules/project-rules.md` 第 12 节，确保源码模式与包模式同步。包模式下禁止通过 `process.cwd()` 读取主题自有文件。

进行包模式冒烟测试时，在临时项目运行 `npx.cmd xiaomais init`，检查 `xiaomais/`、`public/`、项目根依赖和 pnpm 构建脚本许可配置。先运行 `pnpm.cmd check:manifest`、`npx.cmd astro check`、`pnpm.cmd type-check`、`pnpm.cmd build`，再在包模式临时项目运行 `astro build` 和开发服务器冒烟测试。

发布由 `yCENzh/xiaomais` 仓库的 Actions -> Build & Publish 手动触发；合并前使用 `Build and validate, but do not publish` 进行预演。npm 版本以本仓库 `package.json` 的 `version` 为准。
