 ...
 * | 改代码仓 `.gitignore` / git 索引 / `xiaomai.content.json` | 会 | **绝不** |
...
if (options.help) {
	console.log(
		[
			...
			"",
			"This script never modifies code repository's .gitignore, git index, or xiaomai.content.json -- that is content:eject's job.",
		].join("\n"),
	);
	process.exit(0);
}
...
try {
	...
	fail("Resolve content source", error, "Manifest file: xiaomai.content.json");
}
...
if (resolved.mode === "local") {
	fail(
		...
		"In local mode, content is in the code repository, there is no content repository to export to. " +
			...
			"if you already have a content repository, set CONTENT_DIR or declare source in xiaomai.content.json.",
	);
}
...
function exemptionReason(repoRelative) {
	...
	if (resolved.keep.length > 0 && matchesAny(repoRelative, resolved.keep)) {
		return "Declared as code repository private in xiaomai.content.json keep";
	}
	return null;
}
