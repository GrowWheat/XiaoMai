...
export interface OverlayPluginOptions {
	...
	/** Explicit component override map from `XiaoMaisOptions.components`. */
...
/**
 * Vite plugin implementing XiaoMai's component/config override system.
 *
 ...
 * package specifiers such as `xiaomais/collections` and broke them. Now the
 ...
 */
export function xiaomaisOverlay(options: OverlayPluginOptions): Plugin {
	...
	for (const [key, value] of Object.entries(components)) {
		...
		if (!existsSync(target)) {
			throw new Error(
				`[xiaomais] Component override "${key}" points at "${value}", which does not exist ` +
					`(resolved to ${target}).`,
			);
		}
