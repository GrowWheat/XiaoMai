...
function createFixture() {
	const base = mkdtempSync(join(tmpdir(), "xiaomai-eject-"));
...
describe("content eject", () => {
	it("默认只预演，不改动仓库", () => {
		...
		try {
			...
			assert.ok(!existsSync(join(fixture.repo, "xiaomai.content.json")));
		} finally {
	...
	it("--yes 写出指向导出目录的 xiaomai.content.json", () => {
		const fixture = createFixture();
		try {
			runEject(fixture, ["--yes", "--out", fixture.out]);
			const manifest = JSON.parse(
				readFileSync(join(fixture.repo, "xiaomai.content.json"), "utf8"),
			);
