...
function createFixture({ ejected = false } = {}) {
	const root = mkdtempSync(join(tmpdir(), "xiaomai-clean-"));
	fixtures.push(root);
	...
	write(
		...
		[
			...
			"!public/assets/moments/thumbnails/.gitkeep",			"src/assets/fonts/.subset/",			"",
		].join("\n"),
	);
	...
	write(root, "public/assets/moments/thumbnails/.gitkeep", "");	// 主题源码（清理范围之外）
...
describe("content:clean 清理范围与分类", () => {
	...
	it("构建期生成物与 .gitkeep 占位文件被豁免", () => {
		...
		for (const path of [
			"public/assets/moments/thumbnails/a-192.webp",			"src/assets/fonts/.subset/x.woff2",			"public/assets/moments/thumbnails/.gitkeep",		]) {
...
describe("content:clean 备份与熔断", () => {
	...
	it("非 Git 工作区立即熔断，并声明未产生破坏性改动", () => {
		const root = mkdtempSync(join(tmpdir(), "xiaomai-clean-nogit-"));
		fixtures.push(root);
