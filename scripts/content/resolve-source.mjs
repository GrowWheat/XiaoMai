...
/** 代码仓根目录下的内容分离清单文件名。 */
export const MANIFEST_FILE = "xiaomai.content.json";
 ...
 */
export const PROTECTED_PATHS = Object.freeze([	"public/assets/moments/thumbnails/**",
 ...
 * 解析当前生效的内容源。
 *
 * 优先级：`XIAOMAI_CONTENT_SYNC=0` > `CONTENT_DIR` > `CONTENT_REPO_URL` > 清单 `source`。
...
export function resolveContentSource(root = process.cwd()) {
	...
	if (isDisabled(process.env.XIAOMAI_CONTENT_SYNC)) {
		return {
			mode: "local",
			reason: "XIAOMAI_CONTENT_SYNC is explicitly disabled",
			reasonLocation: envOrigins.get("XIAOMAI_CONTENT_SYNC") ?? "environment",
		};
	}
