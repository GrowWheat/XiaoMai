...
//        xiaomai- 前缀、description 非空），正文引用的仓库路径全部真实存在。
// 失败 = 打印问题清单并 exit 1。
...
for (const entry of entries) {
	...
	} else if (!name.startsWith("xiaomai-")) {
		fail(`${entry.name}/SKILL.md：name "${name}" 缺少 xiaomai- 前缀`);
	}
	if (!description) {
...
if (existsSync(readmePath)) {
	...
	for (const match of readme.matchAll(
		/\[(xiaomai-[a-z0-9-]+)\]\((xiaomai-[a-z0-9-]+)\/SKILL\.md\)/g,
	)) {
		if (match[1] !== match[2]) {
