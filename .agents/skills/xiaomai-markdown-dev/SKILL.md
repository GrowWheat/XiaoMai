---
name: xiaomai-markdown-dev
description: "为 XiaoMai 主题开发自定义 Markdown 语法与 remark/rehype 插件——插件所有权边界、注册顺序、语法清单、内容驱动的按需加载、npm 集成同步与测试。当新增或变更 Markdown 扩展、正文插件、生成的组件样式或包模式接线时使用。"
---

# XiaoMai Markdown 扩展开发
...
Markdown 语法和样式修改必须能被包模式 integration 发现。新增语法和 stylesheet pack 时登记到 `src/plugins/markdown/manifest.json`，并在 `xiaomais` 打包仓库同步处理示例内容 rewrite。修改路径或 integration wiring 前阅读 `docs/npm-package-mode.md`、`docs/packaging-contract.md` 和 `rules/project-rules.md` 第 12 节。
