...
test("generateLlmsTxt produces standard llms.txt structure", () => {
	const posts = [
		{
			...
			data: {
				title: "Theme Guide",
				description: "A comprehensive guide to XiaoMai.",
	...
	const txt = generateLlmsTxt({
		posts,
		baseUrl: "https://xiaomai.test",
		config,
		siteTitle: "XiaoMai Blog",
		siteSummary: "An M3E theme for Astro.",
	});

	assert.match(txt, /^# XiaoMai Blog/);
	assert.match(txt, /> An M3E theme for Astro\./);
	assert.match(txt, /## Core Pages/);
	assert.match(txt, /- \[Home\]\(https:\/\/xiaomai\.test\/\): Homepage/);
	assert.match(txt, /## Articles/);
	assert.match(txt, /- \[Theme Guide\]\(https:\/\/xiaomai\.test\/posts\/guide\/\): A comprehensive guide to XiaoMai\./);
	assert.match(txt, /## Full Text Dump/);
	assert.match(txt, /- \[Full Text Archive\]\(https:\/\/xiaomai\.test\/llms-full\.txt\)/);
});
...
test("generateLlmsFullTxt aggregates post metadata and cleaned body into Markdown stream", () => {
	...
	assert.match(fullTxt, /^# XiaoMai - Full Content Archive/);
	assert.match(fullTxt, /## First Post/);
	assert.match(fullTxt, /- \*\*URL\*\*: https:\/\/xiaomai\.test\/posts\/post-1\//);
